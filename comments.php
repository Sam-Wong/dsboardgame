...
<?php if(comments_open()) ://if comment is open ?>
   <li id="add_comment"><a href="#respond"><?php _e('发表评论','monochrome'); ?></a></li>
<?php endif; ?>
   <li id="comment_feed"><a href="<?php bloginfo('comments_rss2_url'); ?>" title="<?php _e('评论 RSS','monochrome'); ?>"><?php _e('评论 RSS','monochrome'); ?></a></li>
  </ul>
...
<?php if(pings_open()) ://if trackback is open ?>
    ...
    <li id="comment_switch" class="comment_switch_active"><a href="javascript:void(0);"><?php _e('评论','monochrome'); ?><?php echo (' ( ' . (count($comments)-count($trackbacks)) . ' )'); ?></a></li>
<?php else ://if comment is closed,show onky number ?>
    <li id="trackback_closed"><?php _e('Trackback are closed','monochrome'); ?></li>
    <li id="comment_closed"><?php _e('评论', 'monochrome'); echo (' (' . (count($comments)-count($trackbacks)) . ')'); ?></li>
<?php endif; ?>
  </ul>
...
<ol class="commentlist">
	...
	?>
<li class="comment"><div class="comment-content"><p><?php _e('暂无评论。','monochrome'); ?></p></div></li>
	<?php
		} 
	?>
</ol>
...
<?php else : ?>
<li class="comment"><div class="comment-content"><p><?php _e('暂无 Trackback','monochrome'); ?></p></div></li>
<?php endif; ?>
</ol>
<?php endif; ?>
<!-- trackback end -->
...
<?php else : ?>
<div id="guest_info">
 <div id="guest_name"><label for="author"><span><?php _e('昵称','monochrome'); ?></span><?php if ($req) _e('[ 必须 ]', 'monochrome'); ?></label><input type="text" name="author" id="author" value="<?php echo $comment_author; ?>" size="22" tabindex="1" <?php if ($req) echo "aria-required='true'"; ?> /></div>
 <div id="guest_email"><label for="email"><span><?php _e('邮箱','monochrome'); ?></span><?php if ($req) _e('[ 必須 ]', 'monochrome'); ?> <?php _e('- 不会被公开'); ?></label><input type="text" name="email" id="email" value="<?php echo $comment_author_email; ?>" size="22" tabindex="2" <?php if ($req) echo "aria-required='true'"; ?> /></div>
 <div id="guest_url"><label for="url"><span><?php _e('站点','monochrome'); ?></span></label><input type="text" name="url" id="url" value="<?php echo $comment_author_url; ?>" size="22" tabindex="3" /></div>
...
<div id="submit_comment_wrapper">
 ...
 <input name="submit" type="submit" id="submit_comment" tabindex="5" value="<?php _e('提交', 'monochrome'); ?>" title="<?php _e('提交', 'monochrome'); ?>" alt="<?php _e('提交', 'monochrome'); ?>" />
</div>
